...
def pyrdf(value, class_type=None, datatype=None, **kwargs):
    ...
    if isinstance(value, rdflib.term.Literal):
        class_type = 'literal'
        datatype = value.datatype or __XSD_STRING__
        if value._language:
            kwargs['lang'] = value._language
        value = value.value
    elif isinstance(value, rdflib.term.URIRef):
        return Uri(value)
